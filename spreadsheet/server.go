...
func (s *Server) populateSyntax() {
	...
	s.Sheet.SetCell("C7", "= map(B7, (x) -> x * x)")
	s.Sheet.SetCell("D7", "= filter(B7, (x) -> x > 2)")
	s.Sheet.SetCell("E7", "= reduce(B7, (acc, x) -> acc + x, 0)")
...
func (s *Server) createUpdateResponse(cell *Cell) UpdateResponse {
	...
	return UpdateResponse{
		...
		Value:   cell.RawValue,
		Display: valStr,
		Error:   func() string { if cell.Error != nil { return cell.Error.Error() }; return "" }(),
	}
}
