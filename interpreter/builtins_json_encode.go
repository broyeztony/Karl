...
func encodeJSONValue(value Value) (interface{}, error) {
	...
	case *Float:
		if math.IsNaN(v.Value) || math.IsInf(v.Value, 0) {
			return nil, &RuntimeError{Message: "toJson cannot encode NaN or Inf"}
		}
		return v.Value, nil
	case *String:
		return v.Value, nil
	...
	case *Map:
		...
		for k, val := range v.Pairs {
			if k.Type != STRING && k.Type != CHAR {
				return nil, &RuntimeError{Message: "toJson only supports map with string keys"}
			}
			enc, err := encodeJSONValue(val)
			if err != nil {
				return nil, err
		...
		return out, nil
	default:
		return nil, &RuntimeError{Message: "toJson does not support " + string(value.Type())}
	}
}
