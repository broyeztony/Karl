...
func decodeJSONValue(value interface{}) (Value, error) {
	...
	case json.Number:
		...
		if strings.ContainsAny(raw, ".eE") {
			...
			if err != nil || math.IsNaN(f) || math.IsInf(f, 0) {
				return nil, recoverableError("fromJson", "fromJson invalid float: "+raw)
			}
			return &Float{Value: f}, nil
		}
		i, err := strconv.ParseInt(raw, 10, 64)
		if err != nil {
			return nil, recoverableError("fromJson", "fromJson integer overflow: "+raw)
		}
	...
	default:
		return nil, recoverableError("fromJson", "fromJson unsupported value")
	}
}
